...
SemaphoreHandle_t ConsoleSemaphore = NULL;            //To control multi-task access
...
static void ulong_to_string(unsigned long num,
	char *str,
	unsigned int base,
	bool alt_digits);
...
static int blockprn_to_buf(int c, void* prn_func)
{
	if (OutputCount < MaxOutCount) {
		...
		if(OutputCount % BUFSIZE == 0) {
										// If the buffer is full write the buffer to function pointer         
...
{
	//if (xSemaphoreTake(ConsoleSemaphore, MS_TO_TICKS(1000)) != pdTRUE) 
	//	return -1;
	
	va_list args; 													// Argument list
	int count = -1; 													// Preset fail to number of characters printed
	if(Console_WriteChar) {
												// Unit has been initialized with output function
